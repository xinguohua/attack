...
3. **E1 = finding-driven 6-stage loop, NOT a pre-baked ablation table.** Canonical attack framework lives in `pids_attack/attack/safemimic_cmd/` (single paper-facing implementation). `experiments/E1_ablation/` is an **experiment track**: it only configures `SafeMimicConfig` variants, invokes `attack/safemimic_cmd/`, and collects JSON — **0 lines of attack logic**. The dependency direction is one-way: `experiments/E1_ablation/` may call the framework, but `attack/safemimic_cmd/` must never import, read, or depend on E1 result folders, grids, or aggregate scripts. E1 findings can drive later framework revisions, but the runtime implementation remains in `attack/`, not in `experiments/`. Stage order matches both paper §-progression and code dependency:
   - **E1.0 Bootstrap (§3 closure + §4 minimal Add)** — verify min closed loop: A0 + 1 Add δ real execution → cached benign trace composition → CDM/SQL → detector → R1/R2/ASR/JSON
...
4. **E2 full attack (code updated, rerun needed).** `pids_attack/experiments/E2_attack/` contains the SafeMimic-CMD main attack grid. Existing JSON/CSV results are old graph-summary oracle artifacts; current code uses cached-mixed node-level GT oracle and must be rerun before paper claims.
...
attack/ (algorithm) → attack/framework/oracle.py (query_with_validation_mixed)
  → range/cached_mixed.py (cached benign trace + fresh marker(A0⊕δ) composition)
    → range/mixed_workload.py::collect_attack_query_trace (docker exec + strace + R1/R2)
      → range/converter.py (composed clean.strace → CDM SQL)
...
- `attack/framework/oracle.py` — attack-time black-box oracle; executes fresh `A0⊕δ`, composes it with a cached benign trace, then calls `detection/inference/` for detector inference.
...
- `benign_cache.py` — selects pre-collected benign traces from `detection/data/training_traces/benign_*.strace` for attack-time detector background.
- `cached_mixed.py` — attack oracle composition path: fresh marker-preserving `A0⊕δ` trace + cached benign trace → `raw.strace` / `clean.strace` / SQL / GT. R1/R2 always comes from the fresh attack execution; cached benign only supplies detector context.
- `mixed_workload.py` — shared batch workload helpers. E0 still uses live benign+A0 mixed collection; attack oracle uses `collect_attack_query_trace()` from this module.
...
- **Per-query trace files**: attack queries write to `/tmp/pids_attack_queries/<scenario>_<uuid>/` with `attack_query/raw.strace` for the fresh A0⊕δ execution and composed `raw.strace / clean.strace / clean.strace.sql / gt.json` for detector input. Don't reuse UUIDs across queries.
