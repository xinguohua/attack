...
# Adapter-side cleanup is limited to E0 harness controller artifacts.  Generic
# OS/resource filtering and executable-file post-filters are intentionally not
# enabled by default because they are detector-policy choices, not GT cleanup.
SYSTEM_RESOURCE_ALERT_FILTER_DETECTORS = {"magic", "orthrus", "threatrace"}
_E0_BENIGN_STOP_FILE_RE = re.compile(r"^/(?:private/)?tmp/e0_[^/]+/benign\.stop$")
_E0_BATCH_CONTROLLER_SUBJECT_RE = re.compile(
    r"(?:RUN_DIR|STOP_FILE|BG_PIDS)=/(?:private/)?tmp/e0_"
    r"|/(?:private/)?tmp/e0_[^/\s]+/(?:benign\.stop|bg\.pids)"
)


def default_system_resource_alert_filter_enabled(detector_name: str) -> bool:
    return detector_name in SYSTEM_RESOURCE_ALERT_FILTER_DETECTORS


def _build_args(detector_name: str, artifact_dir: Optional[str] = None) -> argparse.Namespace:
    ...
    fake_argv = [
        ...
        "--artifact_dir", str(artifact_dir or ARTIFACT_DIR),
    ]
...
def _runtime_use_kmeans(config_use_kmeans: Any, *, threshold_override_active: bool) -> bool:
    """Whether runtime inference should apply KMeans post-processing.

    Orthrus defaults to PIDSMaker's KMeans eval path. A calibrated runtime
    threshold is a different explicit operating point, so it must not be
    overwritten by KMeans after thresholding.
    """
    return bool(config_use_kmeans) and not threshold_override_active


def _node_label_value(label: str, node_type: str) -> str:
    prefix = f"{node_type} "
...
def is_system_resource_alert_noise(node: Dict[str, Any]) -> Optional[str]:
    """Return a suppression reason for E0 harness controller alerts.

    This is an adapter-side post-filter, not a PIDSMaker upstream change.  It is
    deliberately limited to files/commands created by the E0 runner itself to
    control background loops. It does not suppress procfs, resolver traffic,
    runtime libraries, or executable-file alerts.
    """
    node_type = str(node.get("node_type") or "").strip()
    label = str(node.get("label") or "").strip()
    value = _node_label_value(label, node_type)

    if node_type == "subject":
        if _E0_BATCH_CONTROLLER_SUBJECT_RE.search(value):
            return "e0_batch_controller_subject"
        return None

    if node_type == "file":
        if _E0_BENIGN_STOP_FILE_RE.match(value):
            return "e0_benign_stop_file"
        return None

    return None


def suppress_system_resource_alerts(nodes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
    for node in nodes:
        nd = dict(node)
        if int(nd.get("y_pred", 0)) == 1:
            reason = is_system_resource_alert_noise(nd)
            if reason:
                nd["y_pred"] = 0
                nd["post_filtered"] = True
                nd["post_filter_reason"] = reason
        out.append(nd)
    return out
...
class PIDSMakerEngine:
    ...
    def __init__(
        ...
        suppress_system_resource_alerts_enabled: bool = False,
    ):
        self.detector_name = detector_name
        self.model_path = model_path
        self.artifact_dir = artifact_dir
        self.threshold_override = threshold_override
        self.suppress_system_resource_alerts_enabled = suppress_system_resource_alerts_enabled
        self._loaded = False
    ...
    def _ensure_loaded(self):
        ...
        args = _build_args(self.detector_name, artifact_dir=self.artifact_dir)
        ...
        best_dir = (
            ...
            if self.model_path
            else os.path.join(cfg.training._trained_models_dir, "best_model")
        )
        ...
        self._threshold_override_active = self.threshold_override is not None
        # orthrus 用 use_kmeans=True 在 eval 阶段把 threshold compare 覆盖 ——
        # 推理这边也得镜像同样逻辑,否则 orthrus 永远报 0(eval 上 F1=0.29 vs 推理 0)。
        # If a calibrated runtime explicitly supplies threshold_override, prefer
        # that operating point; otherwise the override would be silently ignored
        # by the KMeans post-processing branch.
        self._use_kmeans = _runtime_use_kmeans(
            getattr(cfg.evaluation.node_evaluation, "use_kmeans", False),
            threshold_override_active=self._threshold_override_active,
        )
    ...
    def predict(self, sql_path: str) -> int:
        ...
        Attack/E0/E2 的论文口径不直接用这个函数,而是在
        detection/inference/registry.py + attack/framework/oracle.py 中按
        mixed-run node GT 计算节点级指标。
    ...
    def predict_per_node(self, sql_path: str) -> List[Dict[str, Any]]:
        ...
        for nd, yp in zip(node_list, y_preds):
            ...
            node_type = label_pair[0] if label_pair else ""
            out_list.append({
                "node_index_id": nid,
                "node_type": node_type,
                "label": label,
        ...
        if self.suppress_system_resource_alerts_enabled:
            out_list = suppress_system_resource_alerts(out_list)
        return out_list
...
class _LocalDetector:
    ...
    """
    _engines: Dict[tuple, "PIDSMakerEngine"] = {}
    ...
    def __init__(
        ...
        artifact_dir: Optional[str] = None,
        threshold_override: Optional[float] = None,
        suppress_system_resource_alerts_enabled: Optional[bool] = None,
    ):
        ...
        self.artifact_dir = artifact_dir
        self.threshold_override = threshold_override
        self.suppress_system_resource_alerts_enabled = (
            default_system_resource_alert_filter_enabled(detector_name)
            if suppress_system_resource_alerts_enabled is None
            else bool(suppress_system_resource_alerts_enabled)
        )
    ...
    def _get_engine(self) -> "PIDSMakerEngine":
        cache_key = (
            self.detector_name,
            self.model_path,
            self.artifact_dir or ARTIFACT_DIR,
            self.threshold_override,
            self.suppress_system_resource_alerts_enabled,
        )
        if cache_key not in self._engines:
            self._engines[cache_key] = PIDSMakerEngine(
...
用法:
    python -m detection.training.pidsmaker eval                    # 8 detector 全测
    python -m detection.training.pidsmaker eval orthrus            # 单 detector
    python -m detection.training.pidsmaker eval flash rcaid velox  # 多 detector
"""
import glob
import os
import re
...
def _build_eval_args(detector_name: str):
...
def load_eval_pkl(detector_name: str):
    ...
    import torch
    cfg = _get_yml_cfg_safe(_build_eval_args(detector_name))
...
流程:
  ...
  Step 5.  (可选)跑内部 eval_main 出真实指标矩阵
...
跟推理代码解耦 ——
  推理类 `_LocalDetector` / `SUPPORTED_DETECTORS` 在 detection/training/pidsmaker.py
"""
import argparse
import os
import shutil
...
ARTIFACT_DIR = Path(os.environ.get(
    "PIDSMAKER_ARTIFACT_DIR",
    str(PROJECT_ROOT / "detection" / "training" / "artifacts"),
))
...
# 跟 detection/data/data_prep.py 对齐
...
def step_print_dataset_overview(args):
    ...
    attack_dir = PROJECT_ROOT / "detection" / "data" / "test_traces" / "attack"
    ...
    print(f"    attack:    {attack_dir}/*.strace.sql ({len(list(attack_dir.glob('*.strace.sql')))} 个 scenario, test only)")
...
def step_ingest(args):
    ...
    if not benign_files:
        ...
        print(f"  先跑:python scripts/run.py detect collect --num-collections 30 --parallel 4")
        sys.exit(1)
    print(f"  [check] {len(benign_files)} 份 benign SQL OK")

    cmd = [sys.executable, "-m", "detection.data.data_prep",
...
def step_train(args, detectors):
    ...
    for d in detectors:
        ...
        t0 = time.time()
        cp = subprocess.run(
            [sys.executable, "-u", "pidsmaker/main.py", d, "JUICESHOP", "--cpu",
    ...
    # 不在 step_train 末尾打汇总对比表(step_eval 会调 eval_main 出完整版)。
    # 如果跑了 --skip-eval,想看汇总:`python -m detection.training.pidsmaker eval`
    return failed
...
def step_eval(args, detectors):
    if args.skip_eval:
        ...
        return
    banner("Step 5/5 · PIDSMaker 真实指标报告(eval_main)")
    try:
...
def train_main(argv=None):
    ...
    from detection.data.data_prep import audit_training_data_separation
    traces_dir = PROJECT_ROOT / "detection" / "data" / "training_traces"
    attack_dir = PROJECT_ROOT / "detection" / "data" / "test_traces" / "attack"
    try:
        audit_training_data_separation(str(traces_dir), str(attack_dir))
    except RuntimeError as exc:
        print(f"\n[abort] training data audit failed:\n{exc}")
        sys.exit(1)
...
def main(argv=None):
    ...
    print("usage: python -m detection.training.pidsmaker [train|eval] ...")
    print("public: python scripts/run.py detect train-gnn ...")
    return None
