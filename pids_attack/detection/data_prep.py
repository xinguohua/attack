"""detection/data_prep.py — 数据源无关的灌库 / 时间映射 / 索引修正 / GT 工具。

跟 PIDSMaker 解耦:这一层只产 PIDSMaker 兼容的 PostgreSQL DB,跟具体数据源(JUICESHOP / DARPA / OpTC)无关。

不含 `replicate_sql`(已弃用)—— 数据量靠 `collect_benign.py --num-collections N` 真采集解决。
"""
...
"""detection/data_prep.py — JUICESHOP 灌库主流程。

读 N 份 benign_*.sql + 10 份 attack/*.sql 真采集数据,直接灌进 PostgreSQL。
**不复制(replicate_sql 弃用)** —— 数据量靠 collect_benign.py --num-collections N 真采集解决。
...
def ingest_juiceshop_dataset(
    ...
    attack_dir = os.path.join(traces_dir, "attack")
    ...
    if not benign_sqls:
        raise FileNotFoundError(
            ...
            f"run scripts/run.py detect collect-benign --num-collections 30 --parallel 4 first)"
        )
