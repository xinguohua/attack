...
python pids_attack/scripts/run.py detect threshold-sweep --detectors threatrace
...
Attack mode
  ...
  scripts/run.py attack run           GRABNEL-CMD 主循环 + verbose query 日志
```
...
```
                  ...
                  │  attack/                       │   ← Layer A 算法
                  │  9 个研究问题模块化:             │
                  │   search_space (4) + init (4)  │
                  │   perturbation (8) + accept(4) │
                  │   guidance (6) + state_upd (4) │
                  │   termination (4) + fitness(6) │
                  └───────────┬────────────────────┘
                              │ candidate cmd seq
                              ▼
                  ┌────────────────────────────────┐
                  │ attack/oracle.py               │   ← Layer B
                  │ query_with_validation_strict   │
                  └───────────┬────────────────────┘
                              │ scenario + δ + positions
                              ▼
                  ┌────────────────────────────────┐
                  │ range/checker.py               │   ← Layer C
                  │ execute_with_checks            │
                  │   _interleave → 6 条命令        │
                  │   batch runner + per-cmd strace│
                  │   segmented docker exec        │
                  │   ⇨ trace_<uuid>.strace        │
                  └───────────┬────────────────────┘
                              │ trace 文件路径
                              ▼
                  ...
                  └────────────┬───────────────────┘
                               ...
                               ▼
                       回到 attack/oracle.py,再回 Layer A 更新算法状态
```
...
| **攻击算法主循环** | `attack/grabnel_cmd/algorithm.py:GrabnelCMDAttack.run` | — |
...
| **detector 判 y** | `detection/pidsmaker.py:_LocalDetector.predict` | 150 |
...
pids_attack/
├── detection/
│   ├── collect.py             # benign / attack trace 采集
│   ├── data_prep.py           # PIDSMaker JUICESHOP 灌库逻辑
│   ├── pidsmaker.py           # GNN train / eval / inference adapter
│   ├── rules.py               # G1/G2/G1G2/hybrid rule detectors
...
│   ├── framework/             # AttackScenario / AttackResult / QueryResult / QueryHistory
│   ├── grabnel_cmd/           # 当前唯一攻击算法实现
│       ├── runner.py      # scripts/run.py attack run 调用的 runner
│       ├── algorithm.py   # GrabnelCMDAttack 主循环
│       ├── inner_ga.py    # 内层 GA 生成候选 Δ
│       ├── commit.py      # sequential commit 策略
│       ├── acquisition/   # LCB / EI / Thompson
│       ├── fitness/       # attack / stealth / scalarize / reference
│       └── surrogate/     # WL features + Sparse BLR
│   └── oracle.py              # attack-time black-box oracle
├── scenarios/
...
│   ├── E1_operators/      # P1/P2 mutation validation
│   ├── E2_ablation/       # 组件消融 / sensitivity
│   └── E3_attack/         # GRABNEL 完整主实验
...
> 结构按 `scripts/run.py attack smoke-query` 的 STAGE 0-6 和 `scripts/run.py attack run` 的 GRABNEL 主循环排列。
...
> **δ 扰动属于 attack mode**,不在 PART B。`scripts/run.py attack run` 启动 GRABNEL-CMD,动态生成 (delta_cmds, positions),每轮 query 都会换。
...
- **代码**:`scripts/run.py:stage_6_detector_predict` → `detection/pidsmaker.py:_LocalDetector.predict_per_node` → 「**PIDS oracle —— 推理**」详见下面独立 section
- **关键事实**:每次 predict 真跑 GNN forward(不是关键字 stub,不查训练时 eval pkl);δ 改 SQL 内容 → forward 输出真变化
- **输出**:per-node `y_pred/score`;attack mode 的成功判断不使用整图 `any(y_pred)`,而由 `query_with_validation_strict()` 过滤 GT/attack nodes 后生成

---

### PIDS oracle —— 推理(STAGE 6 / attack mode 用)

输入 PIDSMaker 兼容 SQL → 输出 per-node prediction。E3 attack oracle 的 $y\in\{0,1\}$ 定义为:至少一个 GT/attack node 被 flagged 时 `y=1`,否则 `y=0`。
...
Attack:   scripts/run.py attack run
            └── GrabnelCMDAttack.run
                └── query_with_validation_strict(scenario, δ_cmds, positions, oracle)
                  ├── execute_with_checks       # checker 守门
                  ├── trace_to_pidsmaker        # strace → SQL
                  ├── _LocalDetector.predict_per_node
                  └── filter GT/attack nodes    # QueryResult.y = attack_detected
```
...
| 文件 | 作用 |
|---|---|
| `detection/pidsmaker.py` | `PIDSMakerEngine` 真 forward + `_LocalDetector` cache + `SUPPORTED_DETECTORS` |
| `attack/oracle.py` | `PIDSOracle` + `query_with_validation_strict`(执行+验证+推理唯一入口) |
...
| `query_with_validation_strict(scenario, δ_cmds, positions, oracle)` | `execute_with_checks(checker) → trace_to_pidsmaker → predict_per_node → filter GT/attack nodes`;checker 不过 → `QueryResult.invalid_` 不消耗预算。`attack/grabnel_cmd/runner.py` 和 `experiments/_common.py:make_oracle` 都直接调它 |
...
### PIDS oracle —— 数据采集(`detect collect-benign`)
...
- **代码**:
  - `detection/collect.py` — 采集实现
  - `scripts/run.py detect collect-benign` — 公开入口
...
python pids_attack/scripts/run.py detect collect-benign --num-collections 30 --parallel 4

# 缩短单份时长(yml 默认 1800s 太长,我们一般 300s 够训)
python pids_attack/scripts/run.py detect collect-benign --num-collections 30 --parallel 4 --duration-override 300
```
...
`benign_<i>.sql` 是 `detection/data_prep.py` 后续 ingest 的输入。

---

### PIDS oracle —— 训练(一次性,跑完已落盘)

- **职责**:把 PIDSMaker 8 个 detector 在 JUICESHOP 上各训一份,落盘到 `pids_attack/detection/data/pidsmaker_artifacts/`(~4.6 GB,模型本身 ~80 MB,其余是 5 个 task 的中间 cache),供 STAGE 6 推理 load。**离线一次性**,跟 STAGE 6 完全分离。
...
- **代码**:
  ...
  - `detection/collect.py` — **数据采集**:`detect collect-benign --num-collections N --parallel K` 并行起 K 个 docker 容器各采一份独立 benign,产 `detection/data/training_traces/benign_<i>.sql` × N
  - `detection/data_prep.py` — **灌库层**:读 N 份 `benign_*.sql` + 10 份 `attack/*.sql` → timestamp shift 到 fake-dates → 灌 `pids_postgres` → 写 Ground Truth
...
scripts/run.py detect train-gnn:main()
  ...
  │           └── detection/data_prep.py:ingest_juiceshop_dataset
  │                 ├── 读 benign_*.sql(N 份独立采集)
  │                 ├── 读 attack/*.strace.sql(10 个 scenario)
  ...
  ├── Step 3  step_clean_cache                rm detection/data/pidsmaker_artifacts/{construction,...,evaluation}
  ...
  │             ⑧ triage             后处理,本项目不用
  │
  └── Step 5  step_eval                       subprocess.run(python scripts/run.py detect eval-gnn)
...
- **关键约束**:
  - **不在 PIDSMaker 里塞 replicate_sql**(旧设计是 1 份 benign textual 复制 30 份补量,假数据);新设计是 `collect_benign.py --num-collections 30 --parallel 4` 真采 30 份独立 benign,假数据彻底删
  - 训练数据自采(`scripts/run.py detect collect-benign` 在 docker 容器内跑 7 daemon + 10 scenario,strace 真录)+ JUICESHOP 自带 10 attack scenario
  - 默认落盘 `pids_attack/detection/data/pidsmaker_artifacts/`(项目内,持久);可用 `export PIDSMAKER_ARTIFACT_DIR=<custom>` 改
  - 训练耗时(实测,8 detector):flash ~521s(最久,word2vec)/ kairos ~176s / orthrus ~108s / 其他 < 50s,总 ~17min
  - **JUICESHOP / PIDSMaker fake-date 配置必须对齐两边**:`detection/data_prep.py:TRAIN_DATES/VAL_DATES/TEST_DATES/ATTACK_TO_DATE` ↔ `PIDSMaker/pidsmaker/config/config.py:JUICESHOP.train_dates/val_dates/test_dates/attack_to_time_window`,改一边记得改另一边

- **输出**:`pids_attack/detection/data/pidsmaker_artifacts/` 下 8 个 detector 各自一个 `<cfg-hash>` 目录(互不冲突):

```
pids_attack/detection/data/pidsmaker_artifacts/
...
### PIDS oracle —— 评测(训练完成验收)

- **职责**:跑完上面训练后,看 8 detector 在 test set 上的真实 precision / recall / F1 + 10 个 scenario 各自的命中(PIDSMaker 自己算的指标,我们不参与计算)。

- **代码**:`scripts/run.py detect eval-gnn` — 读训练时落盘的 `scores_model_epoch_<N>.pkl`,出整体 + 10 scenario 拆分指标(被 `train_pidsmaker.py:step_eval` 自动调,也可单跑)

- **逻辑**:

  ```
  $ python scripts/run.py detect eval-gnn                # 全 8 detector
...
- **输出**(本次 30 份真采 + 重训实测,2026-05-11):
  ...
  `scripts/run.py detect eval-gnn` 8 detector × P/R/F1:
...
- **关键代码**:`scripts/run.py:attack_main` + `attack/grabnel_cmd/runner.py` + `attack/grabnel_cmd/algorithm.py:GrabnelCMDAttack.run`
...
Q3. 选命令  → E3 → 用历史信息在大空间选 c        → 验证 Step 3 设计
```
...
**E3 — 历史信息驱动的命令选择(驱动 Step 3 LinUCB)**
...
E3 ──→ 验证 LinUCB + syscall ──→ 验证 Step 3 设计,query 复杂度 O(|Ω|) → O(k)
```
