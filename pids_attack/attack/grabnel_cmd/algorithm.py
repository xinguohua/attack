"""GrabnelCMDAttack — SafeMimic-CMD v3 主算法。
...
class GrabnelCMDAttack(AttackAlgorithm):
    ...
    def __init__(self, cfg: GrabnelConfig) -> None:
        self.cfg = cfg
    ...
    def _compute_fitness(
        ...
        if self.cfg.scalarize == "tcheby":
            s = tchebycheff(L1, L2, beta=self.cfg.beta)
        elif self.cfg.scalarize == "weighted":
    ...
    def _build_warm_reference(self) -> Reference:
        ...
        try:
            from detection.rules import sql_to_cmd_graph as fn
            sql_to_cmd_graph_fn = fn
        except ImportError:
            pass
        return Reference.warm_start_from_benign(
            n=self.cfg.warm_start_n,
