...
§5.3 TODO E2.3 备选:dist_weighted / KDE / GMM(本文件先实现 default k-NN)。
"""
...
def f2_kde(phi_G: np.ndarray, reference: "Reference", k: int = 5,  # noqa: F821
    ...
    """E2.3 备选:KDE 密度比例(简化版,Gaussian kernel on cosine distance)。
...
def f2_gmm(phi_G: np.ndarray, reference: "Reference", k: int = 5) -> float:  # noqa: F821
    """E2.3 备选:GMM 简化版 — 用 inverse-distance 概率,跟 dist_weighted 类似但 softmax。
