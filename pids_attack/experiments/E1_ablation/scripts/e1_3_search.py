"""E1.3 Search structure — §5.3 sequential + §5.4 inner GA ablation.

Variants(对应 `p3_results.md` §3.10):
  full / random

cfg.search_policy 选 dispatch。所有变量都通过同一 attack.safemimic_cmd 框架运行。
"""
...
VARIANTS: Dict[str, Dict[str, Any]] = {
    "full":   {"search_policy": "full"},
    "random": {"search_policy": "random"},
}
...
def main(argv: Optional[Sequence[str]] = None) -> Dict[str, Any]:
    p = argparse.ArgumentParser(description="E1.3 search structure ablation")
    ...
    summary["experiment"] = "E1.3_search"
    write_summary(summary, args.output)
    print(f"[E1.3/{args.variant}] converged={summary['converged']} q={summary['q_used']} |Δ|={summary['delta_len']} wall={wall:.1f}s")
    return summary
