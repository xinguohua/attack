"""E1.5 Acquisition — §5.4 LCB / EI / Thompson ablation.

Variants(对应 `p3_results.md` §3.12):
  lcb / ei / thompson

cfg.acquisition 选 dispatch。所有变量都通过同一 attack.safemimic_cmd 框架运行。
"""
...
def main(argv: Optional[Sequence[str]] = None) -> Dict[str, Any]:
    p = argparse.ArgumentParser(description="E1.5 acquisition ablation")
    ...
    summary["experiment"] = "E1.5_acquisition"
    write_summary(summary, args.output)
    print(f"[E1.5/{args.variant}] converged={summary['converged']} q={summary['q_used']} |Δ|={summary['delta_len']} wall={wall:.1f}s")
    return summary
