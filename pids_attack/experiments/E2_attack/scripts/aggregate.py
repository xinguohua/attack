...
Usage:
    python aggregate.py --root pids_attack/experiments/E2_attack --out figures/E2_attack_summary.csv
"""
...
def _algo_from_file(fname: str) -> str:
    ...
    if not parts:
        return "?"
    return "full" if parts[0] == "grabnel" else parts[0]
...
def print_summary(rows: List[Dict[str, Any]]) -> None:
    ...
    # E2.0 per (detector, algo)
    e30 = [r for r in rows if r["experiment"].startswith("E2.0")]
    if e30:
        print("\n=== E2.0 by (detector, algorithm) ===")
...
def main():
    ...
    p.add_argument("--out", default="figures/E2_attack_summary.csv")
    args = p.parse_args()
