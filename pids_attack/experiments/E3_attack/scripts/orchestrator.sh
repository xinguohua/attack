...
ROOT=/Users/xinguohua/mimicattack/pids_attack/experiments/E3_attack
LOG_DIR=$ROOT/logs
...
snapshot() {
  ...
  tar czf "$outdir/code_snapshot.tgz" \
    ...
    pids_attack/experiments/E3_attack/scripts \
  ...
  for det in g1 g2 g1g2; do
    for scn in "${SCENARIOS[@]}"; do
      for seed in "${SEEDS[@]}"; do
        run_cell random $det $scn $seed $ROOT/E3.0_main_attack/results
      done
    done
  done
  log_stage "A.2" "✅" "done"
...
# ─── STAGE A.3:GNN × grabnel(150)─────────────────
stage_a3() {
  log_stage "A.3" "🔄" "GNN × grabnel"
  for det in magic orthrus threatrace; do
    for scn in "${SCENARIOS[@]}"; do
      for seed in "${SEEDS[@]}"; do
        run_cell grabnel $det $scn $seed $ROOT/E3.0_main_attack/results
      done
    done
  done
  log_stage "A.3" "✅" "done"
...
stage_a5() {
  ...
  for algo in grabnel random; do
    for scn in "${SCENARIOS[@]}"; do
      for seed in "${SEEDS[@]}"; do
        run_cell $algo magic_g1g2 $scn $seed $ROOT/E3.0_main_attack/results
      done
    done
  done
  log_stage "A.5" "✅" "done"
...
stage_i() {
  ...
  PYTHONPATH=pids_attack $PY $ROOT/scripts/aggregate.py \
    --root $ROOT --out $ROOT/figures/E3_attack_summary_final.csv > $ROOT/logs/final_summary.txt 2>&1
  log_stage "I" "✅" "done — see figures/E3_attack_summary_final.csv"
  echo "" >> "$PROG"
